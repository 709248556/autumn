 ...
 * 基于Solr 的文法解析
 */
public class SolrLexerParse extends AbstractLexerParse {
	/**
	 * 实例化 SolrLexerParse 类新实例
	 *
	 * @param expression
	 *            表达式
	 */
	public SolrLexerParse(String expression) {
	 ...
	 * @param delimiter
	 */
	private SolrLexerParse(String expression, String delimiter) {
	...
	protected Token read() {
		...
		try {
			...
			if (isDelimiter(this.delimiter, this.expression.charAt(this.index))) {
				result = this.readDelimiterToken('<', '>', '=');
				if (result != null) {
					return result;
				}
				result = this.readDelimiterToken('>', '=');
				...
				if (result != null) {
					if ("&".equals(result.getText())) {
						throw new ParserDelimiterException(result.getStartIndex(), result.getLength(), "&",
			...
			// 特殊变量
			result = this.readSplitToken('{', '}', Enums.TokenType.VARIABLE, false);
			if (result != null) {
				return result;
			}
	...
	protected Variant createStaticArray(String text, String delimiter, int startIndex, int length) {
		...
		SolrLexerParse scanner = new SolrLexerParse(text, ",;");
		Token token;
