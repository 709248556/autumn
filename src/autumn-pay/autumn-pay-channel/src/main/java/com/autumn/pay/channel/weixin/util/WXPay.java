...
import com.autumn.pay.channel.weixin.util.WeixinPayConstants.SignType;
...
public class WXPay {
    ...
    /**
     * 处理支付通知返回数据，转换成Map对象。return_code为SUCCESS时，验证签名。
     *
     * @param xmlStr 支付通知返回的XML格式数据
     * @return Map类型数据
     * @throws Exception
     */
    public Map<String, String> processPayNotifyResponseXml(String xmlStr) throws Exception {
        String return_code;
        Map<String, String> respData = WXPayUtil.xmlToMap(xmlStr);
        if (respData.containsKey(RETURN_CODE)) {
            return_code = respData.get(RETURN_CODE);
     ...
     * 退款通知返回数据，转换成Map对象。
     *
     * @param xmlStr 退款通知返回的XML格式数据
     * @return Map类型数据
     * @throws Exception
     */
    public Map<String, String> processRefundNotifyResponseXml(String xmlStr) throws Exception {
        String return_code;
        Map<String, String> respData = WXPayUtil.xmlToMap(xmlStr);
        if (respData.containsKey(RETURN_CODE)) {
            return_code = respData.get(RETURN_CODE);
        ...
        if (return_code.equals(WeixinPayConstants.FAIL)) {
            ...
            // 对商户key做md5
            String keyMd5 = HashUtils.md5(this.config.getKey());
            ...
            String decryptInfo = new String(decoded, StandardCharsets.UTF_8);
            // 将返回串转换成 Map
            Map<String, String> reqInfoMap = WXPayUtil.xmlToMap(decryptInfo);

            respData.putAll(reqInfoMap);
            return respData;
        } else {
            throw new Exception(String.format("return_code value %s is invalid in XML: %s", return_code, xmlStr));
        }
