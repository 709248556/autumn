...
public class Sm4 {
	...
	private long potl(long x, int n) {
	...
	private long sm4Lt(long ka) {
		...
		b[3] = sm4Sbox(a[3]);
		bb = getULongBy(b, 0);
		c = bb ^ potl(bb, 2) ^ potl(bb, 10) ^ potl(bb, 18) ^ potl(bb, 24);
		return c;
	}
	...
	private void setKey(long[] sk, byte[] key) {
		...
		long[] k = new long[36];
		int i = 0;
		mk[0] = getULongBy(key, 0);
		mk[1] = getULongBy(key, 4);
		mk[2] = getULongBy(key, 8);
		mk[3] = getULongBy(key, 12);
		k[0] = mk[0] ^ (long) FK[0];
		k[1] = mk[1] ^ (long) FK[1];
		k[2] = mk[2] ^ (long) FK[2];
		k[3] = mk[3] ^ (long) FK[3];
		for (; i < SmUtils.INTEGER_LENGTH_32; i++) {
	...
	private void sm4OneRound(long[] sk, byte[] input, byte[] output) {
		...
		ulbuf[0] = getULongBy(input, 0);
		ulbuf[1] = getULongBy(input, 4);
		ulbuf[2] = getULongBy(input, 8);
		ulbuf[3] = getULongBy(input, 12);
		while (i < SmUtils.INTEGER_LENGTH_32) {
			ulbuf[(i + 4)] = sm4F(ulbuf[i], ulbuf[(i + 1)], ulbuf[(i + 2)], ulbuf[(i + 3)], sk[i]);
			i++;
		}
		putULongBy(ulbuf[35], output, 0);
		putULongBy(ulbuf[34], output, 4);
		putULongBy(ulbuf[33], output, 8);
		putULongBy(ulbuf[32], output, 12);
	}
	...
	public void setSm4KeyEnc(Sm4Context ctx, byte[] key) throws Exception {
		if (ctx == null) {
			throw new Exception("ctx is null!");
		}

		if (key == null || key.length != SmUtils.INTEGER_LENGTH_16) {
	...
	public void setSm4KeyDec(Sm4Context ctx, byte[] key) throws Exception {
		if (ctx == null) {
			throw new Exception("ctx is null!");
		}
		...
		setKey(ctx.sk, key);
		for (i = 0; i < SmUtils.INTEGER_LENGTH_16; i++) {
			swap(ctx.sk, i);
		}
	}
	...
	public byte[] cryptEcb(Sm4Context ctx, byte[] input) throws Exception {
		...
		int len = SmUtils.INTEGER_LENGTH_16;
	...
	public byte[] cryptCbc(Sm4Context ctx, byte[] iv, byte[] input) throws Exception {	
		if (iv == null || iv.length != SmUtils.INTEGER_LENGTH_16) {
		...
		if (ctx.mode == SM4_ENCRYPT) {
			for (; length > 0; length -= len) {
				...
				System.arraycopy(out1, 0, iv, 0, len);
				bous.write(out1);
			}
		} else {
			...
			for (; length > 0; length -= len) {
				...
				bins.read(in);
				System.arraycopy(in, 0, temp, 0, len);
				sm4OneRound(ctx.sk, in, out);
				for (i = 0; i < len; i++) {
					out1[i] = ((byte) (out[i] ^ iv[i]));
				}
				System.arraycopy(temp, 0, iv, 0, len);
				bous.write(out1);
			}
		}
