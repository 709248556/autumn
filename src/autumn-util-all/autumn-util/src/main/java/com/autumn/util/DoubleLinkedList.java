...
public class DoubleLinkedList<E> extends AbstractSequentialList<E>
	...
	@SuppressWarnings("unchecked")
	private DoubleLinkedList<E> superClone() {
		try {
			return (DoubleLinkedList<E>) super.clone();
	...
	@Override
	public Object clone() {
		DoubleLinkedList<E> clone = superClone();
	 ...
	 * <em>fail-fast</em> {@link java.util.Spliterator} over the elements in this list.
	 *
	 * <p>
	 * The {@code Spliterator} reports {@link java.util.Spliterator#SIZED} and
	 * {@link java.util.Spliterator#ORDERED}. Overriding implementations should document the
	 * reporting of additional characteristic values.
	 *
	 * @implNote The {@code Spliterator} additionally reports
	 *           {@link java.util.Spliterator#SUBSIZED} and implements {@code trySplit} to
	...
	public java.util.Spliterator<E> spliterator() {
		return new DefaultSpliterator<>(this, -1, 0);
	}

	/** A customized variant of Spliterators.IteratorSpliterator */
	static final class DefaultSpliterator<E> implements Spliterator<E> {
		static final int BATCH_UNIT = 1 << 10; // batch array size increment
		static final int MAX_BATCH = 1 << 25; // max batch array size;
		final DoubleLinkedList<E> list; // null OK unless traversed
		Node<E> current; // current node; null until initialized
		int est; // size estimate; -1 until first needed
		int expectedModCount; // initialized when est set
		int batch; // batch size for splits

		DefaultSpliterator(DoubleLinkedList<E> list, int est, int expectedModCount) {
			this.list = list;
			this.est = est;
		...
		public java.util.Spliterator<E> trySplit() {
			...
			if (s > 1 && (p = current) != null) {
				...
				batch = j;
				est = s - j;
				return Spliterators.spliterator(a, 0, j, java.util.Spliterator.ORDERED);
			}
			return null;
		}
		...
		public int characteristics() {
			return java.util.Spliterator.ORDERED | java.util.Spliterator.SIZED | java.util.Spliterator.SUBSIZED;
		}
	}
