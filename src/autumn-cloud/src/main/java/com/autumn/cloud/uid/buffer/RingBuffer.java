...
public class RingBuffer {
     ...
     * 日志
     */
    private static final Log LOGGER = LogFactory.getLog(RingBuffer.class);
     ...
     */
    public synchronized boolean put(UID uid) {
        ...
        if (distance == bufferSize - 1) {
            rejectedPutHandler.rejectPutBuffer(this, uid.getUid());
            return false;
        }
    ...
    public UID take() {
        ...
        if (currentTail - nextCursor < paddingThreshold) {
            LOGGER.info(String.format("Reach the padding threshold:%s. tail:%s, cursor:%s, rest:%s", paddingThreshold, currentTail,
    ...
    protected void discardPutBuffer(RingBuffer ringBuffer, long uid) {
        LOGGER.warn(String.format("Rejected putting buffer for uid:%s. %s", uid, ringBuffer));
    }

    /**
     * Policy for {@link RejectedTakeBufferHandler}, throws {@link SystemException} after logging
     */
    protected void exceptionRejectedTakeBuffer(RingBuffer ringBuffer) {
        String msg = String.format("Rejected take buffer. %s", ringBuffer);
        LOGGER.warn(msg);
