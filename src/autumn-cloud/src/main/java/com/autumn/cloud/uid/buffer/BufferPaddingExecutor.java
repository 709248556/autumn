...
public class BufferPaddingExecutor {
     ...
     * 日志
     */
    private static final Log LOGGER = LogFactory.getLog(BufferPaddingExecutor.class);
    ...
    public static final long DEFAULT_SCHEDULE_INTERVAL = 5 * 60L; // 5 minutes
    ...
    public BufferPaddingExecutor(RingBuffer ringBuffer, BufferedUidProvider uidProvider, boolean usingSchedule) {
        ...
        this.lastSecond = new AtomicLong(TimeUnit.MILLISECONDS.toSeconds(Clock.currentTimeMillis()));
    ...
    public void paddingBuffer() {
        LOGGER.info(String.format("Ready to padding buffer lastSecond:%s. %s", lastSecond.get(), ringBuffer));
        // is still running
        if (!running.compareAndSet(false, true)) {
            LOGGER.info(String.format("Padding buffer is still running. %s", ringBuffer));
            return;
        }
        ...
        while (!isFullRingBuffer) {
            List<UID> uidList = uidProvider.provide(lastSecond.incrementAndGet());
            for (UID uid : uidList) {
        ...
        LOGGER.info(String.format("End to padding buffer lastSecond:%s. %s", lastSecond.get(), ringBuffer));
    }
